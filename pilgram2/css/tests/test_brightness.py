...
def test_brightness():
    ...
    assert list(iter(im_b.getdata())) == list(iter(im.getdata()))
...
def test_brightness_1():
    ...
    assert list(iter(im_b.getdata())) == list(iter(im_b2.getdata()))
    assert im_b.size == im.size
    assert im_b.mode == im.mode
...
def test_brightness_0():
    ...
    assert list(iter(im_b.getdata())) == list(iter(black.getdata()))
    assert im_b.size == im.size
    assert im_b.mode == im.mode
...
def test_brightness_hsv():
    ...
    assert list(iter(im_b.getdata())) == list(iter(im_b2_rgb.getdata()))
